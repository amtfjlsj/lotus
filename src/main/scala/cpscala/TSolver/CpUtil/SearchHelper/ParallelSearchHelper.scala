...
import cpscala.TSolver.Model.Constraint.ParallelConstraint.ParallelPropagator
import cpscala.TSolver.CpUtil.{Constants, INDEX}

class ParallelSearchHelper(override val numVars: Int, override val numTabs: Int, val numBitCons: Int, val parallelism: Int) extends SerialSearchHelper(numVars, numTabs) {
  ...
  def submitToPool(c: ParallelPropagator): Unit = {
    println(pool.getActiveThreadCount + 1)
