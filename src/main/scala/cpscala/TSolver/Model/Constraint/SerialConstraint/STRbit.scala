...
/**
  * 这是STRbit使用BitVar作为变量类型的版本（可以处理论域任意大小的变量）
  * 网络预处理时采用STRbit维持网络GAC，
  * 在搜索过程中也采用STRbit维持网络GAC，
  */

class STRbit(val id: Int, val arity: Int, val numVars: Int, val scope: Array[SerialVar], val tuples: Array[Array[Int]], val helper: SerialSearchHelper) extends SerialPropagator[SerialVar] {
  ...
  // 寻找没有支持的值
  def searchSupport(evt: ArrayBuffer[SerialVar]): Boolean = {
