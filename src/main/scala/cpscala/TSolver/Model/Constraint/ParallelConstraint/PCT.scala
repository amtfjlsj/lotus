...
class PCT(val id: Int, val arity: Int, val num_vars: Int, val scope: Array[ParallelVar], val tuples: Array[Array[Int]], val helper: ParallelSearchHelper) extends ParallelPropagator {
  ...
  for (i <- 0 until arity) {
    supports(i) = Array.ofDim[Long](scope(i).size, numBit)
  ...
  override def call(): Unit = {
    println(helper.pool.getActiveThreadCount + 1)
